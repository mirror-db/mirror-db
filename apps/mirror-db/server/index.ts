...
app.get("/api/endpoint-tags", async (c) => c.json(md.EndpointTagList));
app.get("/api/endpoint-tags/:tag", async (c) => {
  const epTag = md.EndpointTagList.find((t) => t.tag === c.req.param("tag"));
  return epTag
    ? c.json(epTag)
...
app.get("/api/endpoints", async (c) => {
  let list = md.EndpointsList.slice();
  const tags = c.req.queries("tag");
  if (tags) list = list.filter((e) => e.tags.some((t) => tags.includes(t)));
  return c.json(list);
});
const epByIdName = (c: Context): md.Endpoint | undefined =>
  md.EndpointsList.find((e) => e.id === c.req.param("id_name")) ||
  md.EndpointsList.find((e) => e.name === c.req.param("id_name"));
...
app.get("/api/servers", async (c) => {
  ...
  if (_ep) {
    const ep =
      md.EndpointsList.find((e) => e.id === _ep) ||
      md.EndpointsList.find((e) => e.name === _ep);
...
app.get("/api/servers/:id", async (c) => {
  const server = md.ServerList.find((s) => s.id === c.req.param("id"));
...
app.get("/api/endpoints/:id_name/stats", async (c) => {
  ...
  const servers = md.ServerList.filter((s) => s.endpoint_name === ep.name);
  const server =
    servers?.find((s) => s.tags.includes(md.ServerTag.Origin)) || servers?.[0];
